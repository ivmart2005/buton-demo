...
export const useFlowerDrag = (
  ...
  const handleMouseDown = (event: React.MouseEvent, allFlowers: FlowerType[]) => {
    if (isGlobalDrag || isRotating || event.button !== 0) return;
      ...
      if (x >= 0 && y >= 0 && x < rect.width && y < rect.height) {
        ...
        if (pixel[3] > 10) {
          if (currentFlower.id === flower.id) {
            setIsDragging(true);
            setDragOffset({
              x: event.clientX - flower.x,
              y: event.clientY - flower.y
            });
          } else {
